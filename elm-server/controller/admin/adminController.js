...
const dtime = equire('time-formater');
const AddressController = require('../addressController');

class Admin extends AddressController{
	constructor(){
	    super()
	...
	async login(req, res){
		...
		form.parse(req, async (err,fields,files) => {
			...
			const {user_name, password, status = 1} = fields;//解构
	...
	async register(req, res){
		...
		form.parse(req, async (err, fields, files) => {
			...
			try{
				const admin = AdminModel.findOne({user_name});
				if(admin){
					consolo.log('该用户已经存在');
					res.send({
						status: 0,
