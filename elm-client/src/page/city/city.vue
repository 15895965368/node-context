<template>
  <div class="city_container">
    ...
    <form class="city_form" v-on:submit.prevent>
      <div>
        <input type="search" name="city" placeholder="输入学校，商务，写字楼" class="city_input input_style" required v-model="inputValue"/>
      </div>
      <div>
...
<script>
  ...
  import {getStore, setStore, removeStore} from '@/service/mUtils'
  export default {
      data(){
        return {
          inputValue:'', // 搜索地址
      ...
      methods:{
        ...
        nextpage(index,geohash){
            ...
            if(history){
              let checkreoeat = false;
              this.placeHistory = JSON.parse(history);
              this.placeHistory.forEach(item => {
                  if(item.geahash == geohash){
                    checkreoeat = true;
                  }
              })
              if(!checkreoeat){
